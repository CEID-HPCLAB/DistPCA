...
void Read_Bed(std::ifstream &in, double *temp3, struct logistics *logg){
	...
	//================================================================
	unsigned int np = (unsigned long long)ceil((double)M/PACK_DENSITY);
...
void Read_Bed_Blocks(std::ifstream& in, unsigned long long np, 
	unsigned int blk_size, double *temp3, unsigned int startval, 
	struct logistics *logg, unsigned char *decbin, unsigned char *readbin, 
	double *norm_tmp, bool *seen_snp, double *norm_precomp){
	int N = logg->M, M = logg->N; int idx; 
	double sd=0, avg=0, s;
