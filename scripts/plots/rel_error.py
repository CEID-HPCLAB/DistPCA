...
approx_df = pd.read_csv(approx_file, sep = r"\s+", engine = "python"); U_approx = approx_df.drop(columns = ["FID"]).values; U_true = np.loadtxt(true_file)

m, k = U_approx.shape; eps = 1e-12
...
num_points = 10;idx = np.linspace(0, m - 1, num_points, dtype = int)
...
for i in range(k):
    y_vals = rel_err[idx, i].copy(); all_zero = y_vals.max() == 0

    y_vals = np.where(y_vals == 0, eps, y_vals)

    ax.plot(idx, y_vals, linestyle = "--", marker = markers[i], linewidth = 2, color = colors[i], label = f"PC{i+1}")
...
ax.set_xlabel("Index"); ax.set_ylabel("Relative Error")
...
ax.legend(loc="lower right", frameon = True, 
            fontsize = 12, ncol = 2, bbox_to_anchor = (0.96, 0.14),
            facecolor = '#1a1a1a', # white for light mode
            edgecolor = 'white', # black for light mode
            framealpha = 1.0, shadow = True,
            labelspacing = 0.09, columnspacing = 1.02
        )

plt.tight_layout()
# plt.savefig("rel_error.pdf", dpi = 600, bbox_inches = "tight", transparent = True)
plt.savefig("rel_error.png", dpi = 600, bbox_inches = "tight", transparent = True)